...
def gen_apb_fabric6(spec):
    """Required: name, timeout_cyc. 6-slave APB fabric, S4 privileged."""
    n   = opt(spec, "name", "apb_fabric6")
    tmo = int(required(spec, "timeout_cyc", "apb_fabric6"))
    code = hdr(n, f"APB3 6-slave fabric, priv-filter S4(WDT), {tmo}-cycle timeout")
    code += f"""\
module {n} #(parameter TIMEOUT_CYC={tmo})(
    input  wire        pclk, presetn,
    input  wire        m_psel, m_penable, m_pwrite,
    ...
    output wire [31:0] s4_pwdata, input wire [31:0] s4_prdata,
    input  wire s4_pready, s4_pslverr,
    output wire s5_psel,s5_penable,s5_pwrite, output wire [11:0] s5_paddr,
    output wire [31:0] s5_pwdata, input wire [31:0] s5_prdata,
    input  wire s5_pready, s5_pslverr
);
    wire dec0=m_psel&&(m_paddr[31:12]==20'h00000);
    wire dec1=m_psel&&(m_paddr[31:12]==20'h00001);
    wire dec2=m_psel&&(m_paddr[31:12]==20'h00002);
    wire dec3=m_psel&&(m_paddr[31:12]==20'h00003);
    wire dec4=m_psel&&(m_paddr[31:12]==20'h00004);
    wire dec5=m_psel&&(m_paddr[31:12]==20'h00005);
    wire priv=m_pprot[0];
    wire priv_err=dec4&&!priv;
    wire miss=m_psel&&!(dec0||dec1||dec2||dec3||dec4||dec5);
    reg [4:0] tcnt; reg terr;
    always @(posedge pclk or negedge presetn)
        if (!presetn) begin tcnt<=0; terr<=0; end
        else if (!m_psel||m_pready) begin tcnt<=0; terr<=0; end
    ...
    wire s0_ok=dec0, s1_ok=dec1, s2_ok=dec2, s3_ok=dec3;
    wire s4_ok=dec4&&priv, s5_ok=dec5;
    assign s0_psel=s0_ok; assign s1_psel=s1_ok; assign s2_psel=s2_ok;
    assign s3_psel=s3_ok; assign s4_psel=s4_ok; assign s5_psel=s5_ok;
    assign s0_penable=m_penable; assign s1_penable=m_penable;
    assign s2_penable=m_penable; assign s3_penable=m_penable;
    assign s4_penable=m_penable; assign s5_penable=m_penable;
    assign s0_pwrite=m_pwrite; assign s1_pwrite=m_pwrite; assign s2_pwrite=m_pwrite;
    assign s3_pwrite=m_pwrite; assign s4_pwrite=m_pwrite; assign s5_pwrite=m_pwrite;
    assign s0_paddr=m_paddr[11:0]; assign s1_paddr=m_paddr[11:0];
    assign s2_paddr=m_paddr[11:0]; assign s3_paddr=m_paddr[11:0];
    assign s4_paddr=m_paddr[11:0]; assign s5_paddr=m_paddr[11:0];
    assign s0_pwdata=m_pwdata; assign s1_pwdata=m_pwdata; assign s2_pwdata=m_pwdata;
    assign s3_pwdata=m_pwdata; assign s4_pwdata=m_pwdata; assign s5_pwdata=m_pwdata;
    always @(*)
        if      (s0_ok) m_prdata=s0_prdata;
        else if (s1_ok) m_prdata=s1_prdata;
        else if (s2_ok) m_prdata=s2_prdata;
        ...
        else if (s5_ok) m_prdata=s5_prdata;
        else            m_prdata=32'hDEAD_BEEF;
    wire err_cond=miss|priv_err|terr;
    wire slv_rdy=(s0_ok?s0_pready:1'b1)&(s1_ok?s1_pready:1'b1)&
                 (s2_ok?s2_pready:1'b1)&(s3_ok?s3_pready:1'b1)&
                 (s4_ok?s4_pready:1'b1)&(s5_ok?s5_pready:1'b1);
    assign m_pready  = err_cond ? 1'b1 : slv_rdy;
    assign m_pslverr = miss|priv_err|terr|
                       (s0_ok&s0_pslverr)|(s1_ok&s1_pslverr)|(s2_ok&s2_pslverr)|
                       (s3_ok&s3_pslverr)|(s4_ok&s4_pslverr)|(s5_ok&s5_pslverr);
endmodule
"""
    return {f"{n}.v": code}
