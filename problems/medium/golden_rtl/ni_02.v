...
module ni_02 #(
    ...
    // Latch D-channel response (tl_d_valid fires in S_WAIT; bvalid/rvalid fires in S_IDLE)
    reg r_done;
    reg [DATA_W-1:0] r_rdata_lat;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            r_done<=0; r_rdata_lat<=0;
        end else begin
            // Latch D response when it arrives in S_WAIT
            if (st==S_WAIT && tl_d_valid) begin
                r_done <= 1;
                r_rdata_lat <= tl_d_data;
            end
            // Clear done when handshake acknowledged
            if ((axi_bvalid && axi_bready) || (axi_rvalid && axi_rready))
                r_done <= 0;
            case(st)
                S_IDLE: begin
                    if (axi_awvalid && axi_wvalid) begin
                        r_addr<=axi_awaddr; r_wdata<=axi_wdata; r_mask<=axi_wstrb;
                        is_write<=1; st<=S_SEND; r_done<=0;
                    end else if (axi_arvalid && !axi_awvalid) begin
                        r_addr<=axi_araddr; is_write<=0; st<=S_SEND; r_done<=0;
                    end
                end
